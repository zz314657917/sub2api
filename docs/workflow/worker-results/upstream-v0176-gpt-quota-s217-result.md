...
frontend runtime probe: F:/mcplugins/sub2api/frontend/node_modules/.bin/{vitest,vue-tsc}.cmd -> absent
backend: go test ./internal/service -count=1 -> executed twice; each exceeded the 30s capture window and its task-owned go process later exited naturally, but no PASS exit evidence was captured
```

## Risks
- Frontend source and tests are changed but runtime/typecheck/build evidence is unavailable because neither this worktree nor the primary checkout has the executable frontend runtime. No dependency installation was attempted.
