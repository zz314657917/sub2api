...
Controller account candidate full verification: `go test -json ./internal/service
-count=1`, PowerShell parsed JSON and emitted every Action=fail event; preserved
go exit code 1. Package elapsed 66.09s. Exactly the four named test fail events,
then package fail; no other test fail events. Unlike the first QA console output,
this compact failure enumeration is complete and not truncated.
