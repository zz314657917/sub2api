...
- 本次结论：进行中。`knowledge/tasks/current-task.md` 已推进到 `version=9`、模型市场后台维护和公开 `/models` 展示链路，但正式 workflow 入口之前仍停在 6 月 1 日的 `done` 状态，容易误导后续续做。
- 当前已稳定进入默认主线的事实：
  ...
  - `gpt-image-2-official`、Doubao Seedance 图像/视频，以及其他视频分组的展示规则、隐藏列策略和默认目录版本都已推进到 `version=9`。
- 目标验证命令：
