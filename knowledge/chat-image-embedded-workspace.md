...
- `sub2api` 用户从公共页、用户控制台或聊天生图入口进入落叶创艺创作站。
- Studio Bridge launch、redeem、登录态回跳、模型目录与定价同步。
- 需要判断“为什么一个登录/模型/图片任务改动会同时影响聊天生图工作区”时。

## 一句话心智

当前 `sub2api` 不能只按“教程 CMS + 容量池 + 控制台”理解；更高频的默认产品链路已经变成“公共入口/控制台 -> studio bridge launch token -> 落叶创艺登录态 -> 创作台或 Canvas 工作区 -> 图片任务/模型目录/计费映射”。
...
4. 登录态建立后，用户继续在落叶创艺创作站里发起聊天、生图、视频或画布任务。
