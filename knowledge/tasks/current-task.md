...
- 2026-08-07 上游 `dbb42881c` 已按行为级最小适配合入本地，提交 `d06e7fe64`：新增
  ...
  `go test ./internal/service -count=1` 和目标 identity/manifest 回归均通过。该批已随
  2026-08-07 主线整理推送到 `origin/main@5d1117c09`。
...
- 2026-08-07 上游 `7d38e6712` 已与其前置 `40b8f04a6` 按行为级最小适配完成：新增有界的进程内
  ...
  OpenAI/failover handler、server 编译探针、gofmt 和 Git integrity 均已通过；该批已随
  2026-08-07 主线整理推送到 `origin/main@5d1117c09`，未部署、未调用真实上游账号。QA：
...
- v0.1.171 选择性上游整合已收口到本地 `main`：S181-S201 分为三组行为提交
  ...
  `09f333d6b`，`haoai.png` 资源为 `fbcdc7b6f`。上述既有批次及本轮 S203 已于 2026-08-07 整理后
  推送到 `origin/main@5d1117c09`；没有部署、数据库或生产操作。
