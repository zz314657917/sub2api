...
var auditActionOverrides = map[string]string{
	...
	"POST /api/v1/admin/prompt-audit/policy/rollback":         "admin.prompt_audit.policy.rollback",
	"PUT /api/v1/admin/prompt-audit/policy/draft":             "admin.prompt_audit.policy.draft.update",
	"POST /api/v1/admin/prompt-audit/policy/publish":          "admin.prompt_audit.policy.publish",
	"POST /api/v1/admin/prompt-audit/policy/shadow":           "admin.prompt_audit.policy.shadow",
...
var auditBodyOmittedRoutes = map[string]struct{}{
	...
	"POST /api/v1/admin/prompt-audit/policy/rollback":           {},
	"PUT /api/v1/admin/prompt-audit/policy/draft":               {},
	"POST /api/v1/admin/prompt-audit/policy/publish":            {},
	"POST /api/v1/admin/prompt-audit/policy/shadow":             {},
...
func NewAuditLogMiddleware(auditService *service.AuditLogService) AuditLogMiddleware {
	return AuditLogMiddleware(func(c *gin.Context) {
		...
		if _, omit := auditBodyOmittedRoutes[routeKey]; omit {
			...
			// Always restore the body, including read errors. A failed audit read
			// must not turn into a different application request or an empty bind.
			...
			if err == nil {
				...
				bodyRedacted = "<body omitted: read error>"
			}
		}
