...
func NewStepUpAuthMiddleware(
	...
	grantChecker, userReader := stepUpDepsOrNil(totpService, userService)
	return StepUpAuthMiddleware(stepUpAuth(grantChecker, userReader, stepUpSettingsOrNil(settingService)))
}

func stepUpDepsOrNil(
	totpService *service.TotpService,
	userService *service.UserService,
) (stepUpGrantChecker, stepUpUserReader) {
	var grantChecker stepUpGrantChecker
	if totpService != nil {
		grantChecker = totpService
	}
	var userReader stepUpUserReader
	if userService != nil {
		userReader = userService
	}
	return grantChecker, userReader
}
...
func enforceStepUp(c *gin.Context, grantChecker stepUpGrantChecker, userReader stepUpUserReader, settings stepUpSettingReader) bool {
	...
	if grantChecker == nil || userReader == nil {
		// A missing security dependency must never turn the sensitive route into
		// an accidental allow; fail closed with a retryable service error.
