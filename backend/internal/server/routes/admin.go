...
func registerAuditLogRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	// Clear performs an inline TOTP verification and always writes a trace row.
...
func registerCafeRoomRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	rooms := admin.Group("/cafe/rooms")
	{
		...
		rooms.POST("/:id/reset-quotas", h.Admin.CafeRoom.ResetRoomQuotas)
		rooms.POST("/:id/open-round", h.Admin.CafeRoom.OpenRound)
		rooms.POST("/:id/pause-round", h.Admin.CafeRoom.PauseRound)
		rooms.POST("/reset-quotas", h.Admin.CafeRoom.ResetAllQuotas)
	}
...
func registerGroupRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	groups := admin.Group("/groups")
	{
		...
		groups.PUT("/:id/account-priorities", h.Admin.Group.BatchSetGroupAccountPriorities)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/batch-share-status", h.Admin.Account.BulkSetShareStatus)
		...
		accounts.POST("/:id/share-status", h.Admin.Account.SetShareStatus)
		...
		accounts.POST("/:id/refresh-quota", h.Admin.Account.RefreshQuota)
...
func registerTutorialRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
