...
WITH state_values AS (
 SELECT COUNT(*) = 1 AND MAX(timezone_name) = $3 AND MAX(closed_before) <= $4::date AS valid, MAX(closed_before) AS closed_before, MAX(retained_from) AS retained_from FROM usage_group_rollup_state WHERE id = 1
), state AS (
 SELECT CASE WHEN valid THEN closed_before ELSE DATE '1970-01-01' END AS closed_before, CASE WHEN valid THEN retained_from ELSE TIMESTAMPTZ '1970-01-01 00:00:00+00' END AS retained_from, CASE WHEN valid THEN closed_before::timestamp AT TIME ZONE $3::text ELSE TIMESTAMPTZ '1970-01-01 00:00:00+00' END AS tail_start, valid FROM state_values
), historical AS (
...
func (r *dashboardAggregationRepository) syncGroupUsageRollupsInTx(ctx context.Context, todayStart time.Time) error {
	...
	if err := scanSingleRow(ctx, r.sql, `SELECT closed_before::text, retained_from, timezone_name FROM usage_group_rollup_state WHERE id = 1 FOR UPDATE`, nil, &closedBefore, &previousRetainedFrom, &stateTimezoneName); err != nil {
		return fmt.Errorf("read group usage rollup watermark: %w", err)
	}
	todayDate, timezoneName := service.GroupUsageDate(todayStart), service.GroupUsageTimezoneName()
	...
	if _, err := r.sql.ExecContext(ctx, `UPDATE usage_group_rollup_state SET closed_before = $1::date, retained_from = $2, timezone_name = $3, updated_at = NOW() WHERE id = 1`, todayDate, retainedFrom, timezoneName); err != nil {
		return fmt.Errorf("publish group usage rollup watermark: %w", err)
	}
	return nil
}
...
func lockGroupUsageRollupState(ctx context.Context, tx *sql.Tx) error {
	var id int16
	if err := tx.QueryRowContext(ctx, `SELECT id FROM usage_group_rollup_state WHERE id = 1 FOR UPDATE`).Scan(&id); err != nil {
...
func invalidateGroupUsageRollupsAt(ctx context.Context, tx *sql.Tx, affectedAt time.Time) error {
	_, err := tx.ExecContext(ctx, `UPDATE usage_group_rollup_state SET closed_before = LEAST(closed_before, ($1::timestamptz AT TIME ZONE $2::text)::date), updated_at = NOW() WHERE id = 1`, affectedAt.UTC(), service.GroupUsageTimezoneName())
	return err
}

// TryAcquireGroupUsageRollupLeaderLock holds a PostgreSQL advisory lock on a
// dedicated connection until the returned release function is called. Non-DB
// executors are used by unit fakes and retain the single-instance behavior.
func (r *dashboardAggregationRepository) TryAcquireGroupUsageRollupLeaderLock(ctx context.Context, key int64) (release func(), acquired bool, err error) {
	db, ok := r.sql.(*sql.DB)
	if !ok {
	...
	return func() {
		...
		defer cancel()
		_, _ = conn.ExecContext(releaseCtx, "SELECT pg_advisory_unlock($1)", key)
		_ = conn.Close()
	}, true, nil
}
