...
func (r *compositeModelRouteRepository) ListByGroup(ctx context.Context, groupID int64, includeDisabled bool) ([]service.CompositeModelRoute, error) {
	query := clientFromContext(ctx, r.client).CompositeModelRoute.Query().
	...
	if !includeDisabled {
		query = query.Where(compositemodelroute.EnabledEQ(true))
	}
	rows, err := query.All(ctx)
	if err != nil {
		return nil, err
	}
	routes := make([]service.CompositeModelRoute, 0, len(rows))
	for _, row := range rows {
		routes = append(routes, *compositeModelRouteEntityToService(row))
	}
	return routes, nil
}
