...
func (s *UserRepoSuite) TestDeductBalance_InsufficientFunds() {
	...
	got, err := s.repo.GetByID(s.ctx, user.ID)
	s.Require().NoError(err)
	s.Require().InDelta(5.0, got.Balance, 1e-6, "insufficient deduction must not change balance")
}
...
func (s *UserRepoSuite) TestCRUD_And_Filters_And_AtomicUpdates() {
	...
	s.Require().InDelta(7.5, gotOverdraft.Balance, 1e-6, "insufficient deduction must not change balance")
