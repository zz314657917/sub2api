...
func TestPelicanReviewResultDetailUsesAllowedGroups(t *testing.T) {
	...
	defer db.Close()
	now := time.Now()
	columns := []string{"id", "plan_id", "group_id", "account_id", "model_id", "prompt_version", "status", "error_message", "latency_ms", "char_count", "min_chars", "started_at", "finished_at", "html"}
	mock.ExpectQuery(".*").WithArgs(int64(88), sqlmock.AnyArg()).WillReturnRows(sqlmock.NewRows(columns).AddRow(int64(88), int64(1), int64(7), int64(3), "gpt-test", "pelican-v1", "success", "", int64(1), 100, 100, now, now, "<html>x</html>"))
