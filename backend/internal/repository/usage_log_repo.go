...
func (r *usageLogRepository) createSingle(ctx context.Context, sqlq sqlExecutor, log *service.UsageLog) (bool, error) {
	...
	query := `
		...
		daily_stats AS (
			...
			SELECT
				user_id,
				(created_at AT TIME ZONE $51)::date AS usage_date,
				1 AS requests,
				COALESCE(input_tokens + output_tokens + cache_creation_tokens + cache_read_tokens, 0) AS tokens,
				actual_cost,
				CASE
					WHEN EXTRACT(HOUR FROM created_at AT TIME ZONE $51) >= 0
						AND EXTRACT(HOUR FROM created_at AT TIME ZONE $51) < 6 THEN 1
					ELSE 0
