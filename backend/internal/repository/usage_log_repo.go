...
func appendUsageLogBillingModeWhereConditionWithAlias(conditions []string, args []any, billingMode, alias string) ([]string, []any) {
...
func appendUsageLogBillingModeQueryFilter(query string, args []any, billingMode, alias string) (string, []any) {
...
func appendUsageLogModelWhereCondition(conditions []string, args []any, model, source string) ([]string, []any) {
...
func appendUsageLogModelQueryFilter(query string, args []any, model, source string) (string, []any) {
	if strings.TrimSpace(source) == "" {
...
func (r *usageLogRepository) createSingle(ctx context.Context, sqlq sqlExecutor, log *service.UsageLog) (bool, error) {
	...
	query := `
		...
		daily_stats AS (
			...
			SELECT
				user_id,
				(created_at AT TIME ZONE $59)::date AS usage_date,
				1 AS requests,
				...
				actual_cost,
				CASE
					WHEN EXTRACT(HOUR FROM created_at AT TIME ZONE $59) >= 0
						AND EXTRACT(HOUR FROM created_at AT TIME ZONE $59) < 6 THEN 1
					ELSE 0
	...
	query := fmt.Sprintf(`
		WITH user_spend AS (
			SELECT
				...
				COALESCE(SUM(stats.input_tokens), 0)::bigint as input_tokens,
				COALESCE(SUM(stats.output_tokens), 0)::bigint as output_tokens,
				COALESCE(SUM(stats.cache_creation_tokens), 0)::bigint as cache_creation_tokens,
				COALESCE(SUM(stats.cache_read_tokens), 0)::bigint as cache_read_tokens,
...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	conditions, args = appendUpstreamModelMismatchWhereCondition(conditions, args, filters.UpstreamModelMismatch, "")
...
func shouldUseFastUsageLogTotal(filters UsageLogFilters) bool {
	...
	return filters.UserID == 0 && filters.APIKeyID == 0 && filters.AccountID == 0 && filters.UpstreamModelMismatch == nil
}
...
func (r *usageLogRepository) getUsageTrendWithFilters(ctx context.Context, startTime, endTime time.Time, granularity string, userID, apiKeyID, accountID, groupID int64, model, modelSource string, requestType *int16, stream *bool, billingType *int8, billingMode string, upstreamModelMismatch, nativeCompactionV2 *bool) (results []TrendDataPoint, err error) {
	if upstreamModelMismatch == nil && nativeCompactionV2 == nil && shouldUsePreaggregatedTrend(granularity, userID, apiKeyID, accountID, groupID, model, requestType, stream, billingType, billingMode) {
	...
	query, args = appendUpstreamModelMismatchQueryFilter(query, args, upstreamModelMismatch, "")
...
func (r *usageLogRepository) getGroupStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, billingMode string, upstreamModelMismatch, nativeCompactionV2 *bool) (results []usagestats.GroupStat, err error) {
	...
	query, args = appendUpstreamModelMismatchQueryFilter(query, args, upstreamModelMismatch, "ul")
...
// GetAllGroupUsageSummary returns service-timezone today, yesterday, and retained total costs.
...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	endpoints, endpointErr := r.getEndpointStatsByColumnWithFilters(ctx, "inbound_endpoint", start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode)
	if endpointErr != nil {
		logger.LegacyPrintf("repository.usage_log", "GetEndpointStatsWithFilters failed in GetStatsWithFilters: %v", endpointErr)
		endpoints = []EndpointStat{}
	}
	upstreamEndpoints, upstreamEndpointErr := r.getEndpointStatsByColumnWithFilters(ctx, "upstream_endpoint", start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode)
	...
	endpointPaths, endpointPathErr := r.getEndpointPathStatsWithFilters(ctx, start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode)
...
func appendNativeCompactionV2WhereCondition(conditions []string, args []any, value *bool, alias string) ([]string, []any) {
	...
	return conditions, append(args, *value)
}
...
// appendUpstreamModelMismatchWhereCondition preserves SQL three-valued logic:
// false means IS FALSE and never includes rows where the audit field is NULL.
func appendUpstreamModelMismatchWhereCondition(conditions []string, args []any, mismatch *bool, alias string) ([]string, []any) {
	if mismatch == nil {
		return conditions, args
	}
	column := "upstream_model_mismatch"
	if alias != "" {
		column = alias + "." + column
	}
	return append(conditions, upstreamModelMismatchCondition(column, *mismatch)), args
}

func appendUpstreamModelMismatchQueryFilter(query string, args []any, mismatch *bool, alias string) (string, []any) {
	conditions, unchangedArgs := appendUpstreamModelMismatchWhereCondition(nil, args, mismatch, alias)
	if len(conditions) == 0 {
		return query, unchangedArgs
	}
	return query + " AND " + conditions[0], unchangedArgs
}
