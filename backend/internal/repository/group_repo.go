...
func createGroupRecord(ctx context.Context, client *dbent.Client, groupIn *service.Group) error {
	...
	builder := client.Group.Create().
		...
		SetRoutingScope(groupIn.EffectiveRoutingScope()).
		SetSortOrder(groupIn.SortOrder).
		SetIsExclusive(groupIn.IsExclusive).
		SetStatus(groupIn.Status).
		SetAccessMode(groupIn.AccessMode).
		...
		SetModelMatchPatterns(service.NormalizeGroupModelMatchPatterns(groupIn.ModelMatchPatterns)).
...
func (r *groupRepository) DeleteCascade(ctx context.Context, id int64) ([]int64, error) {
	...
	// 4. Soft-delete composite routes owned by this group.
