...
// Claude Code normally removes its client-side long-context selector before
// sending a provider request. Normalize leaked trailing selectors, including
// duplicated and case-variant forms, while preserving a suffix-only model.
...
func sanitizeAnthropicBodyForBetaTokens(body []byte, anthropicBetaHeader string) ([]byte, bool) {
	...
	changed := false
	strip := func(field string, requiredTokens ...string) {
		...
		if err != nil {
			logger.LegacyPrintf("service.gateway",
				"[BetaFieldSanitize] sjson.DeleteBytes(%s) failed: %v (body len=%d). "+
...
// NormalizeGLMOpenAIReasoningEffort rewrites OpenAI-style reasoning effort
// values to the GLM native high/max scale.
