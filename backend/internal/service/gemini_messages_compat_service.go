...
// AntigravityGeminiModelIDs returns effective Gemini mapping keys for schedulable
// Antigravity accounts. Gemini mixed routes require the account opt-in.
...
func (s *GeminiMessagesCompatService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*ForwardResult, error) {
	...
	for attempt := 1; attempt <= geminiMaxRetries; attempt++ {
		...
		// 错误策略优先：匹配则跳过重试直接处理。
		if policy, rebuilt := s.checkErrorPolicyInLoop(ctx, account, resp); policy != ErrorPolicyNone {
			resp = rebuilt
			errorPolicy = policy
			break
		} else {
	...
	if resp.StatusCode >= 400 {
		...
		if s.rateLimitService != nil {
			if errorPolicy == ErrorPolicyNone {
				errorPolicy = s.rateLimitService.CheckErrorPolicy(ctx, account, resp.StatusCode, respBody)
			}
			switch errorPolicy {
...
func (s *GeminiMessagesCompatService) checkErrorPolicyInLoop(
	...
	return s.rateLimitService.CheckErrorPolicy(ctx, account, resp.StatusCode, body), rebuilt
}
...
func geminiPoolModeRetryableOnSameAccount(account *Account, statusCode int) bool {
...
func isClaudeWebSearchToolMap(tool map[string]any) bool {
	...
	// Function names are client-owned. Promote only explicitly typed server-side
	// search tools to Gemini's built-in Google Search tool.
