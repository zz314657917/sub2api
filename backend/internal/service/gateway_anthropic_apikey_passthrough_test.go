...
func TestGatewayService_AnthropicAPIKeyBearerAuthScheme(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	msgReq, err := svc.buildUpstreamRequest(
		context.Background(), c, account, []byte(`{"model":"gpt-oss:20b","messages":[]}`), "ollama-key", "api_key", "gpt-oss:20b", false, false,
	)
	require.NoError(t, err)
	require.Equal(t, "Bearer ollama-key", getHeaderRaw(msgReq.Header, "authorization"))
	require.Empty(t, getHeaderRaw(msgReq.Header, "x-api-key"))
	...
	countReq, err := svc.buildCountTokensRequest(
		context.Background(), c, account, []byte(`{"model":"gpt-oss:20b","messages":[]}`), "ollama-key", "api_key", "gpt-oss:20b", false,
	)
	require.NoError(t, err)
	require.Equal(t, "Bearer ollama-key", getHeaderRaw(countReq.Header, "authorization"))
	require.Empty(t, getHeaderRaw(countReq.Header, "x-api-key"))
