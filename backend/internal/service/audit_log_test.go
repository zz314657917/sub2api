...
func (r *auditLogMemoryRepository) BatchInsert(_ context.Context, logs []*AuditLog) (int64, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.calls = append(r.calls, "batch")
	if r.batchInsertErr != nil {
		return 0, r.batchInsertErr
	}
...
func (r *auditLogMemoryRepository) ClearAll(_ context.Context, log *AuditLog) (int64, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if log != nil {
		...
		log.Extra["deleted_rows"] = deleted
		copy := *log
		r.logs = append(r.logs, &copy)
	}
	return deleted, nil
...
func (r *auditLogMemoryRepository) List(context.Context, *AuditLogFilter) (*AuditLogList, error) {
	return &AuditLogList{}, nil
}

func (r *auditLogMemoryRepository) GetByID(context.Context, int64) (*AuditLog, error) {
	return nil, ErrAuditLogNotFound
}

func (r *auditLogMemoryRepository) DeleteBefore(context.Context, time.Time, int) (int64, error) {
	return 0, nil
}

func (r *auditLogMemoryRepository) snapshot() ([]*AuditLog, []string) {
	r.mu.Lock()
	defer r.mu.Unlock()
	logs := append([]*AuditLog(nil), r.logs...)
	calls := append([]string(nil), r.calls...)
	return logs, calls
}

func (r *auditLogMemoryRepository) setBatchInsertError(err error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.batchInsertErr = err
}
...
func TestAuditLogServiceClearAllFlushesPendingBeforeTruncate(t *testing.T) {
	repo := &auditLogMemoryRepository{}
	svc := NewAuditLogService(repo, nil)
	svc.Start()
	defer svc.Stop()

	svc.Record(&AuditLog{Action: "admin.account.update"})
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()

	deleted, err := svc.ClearAll(ctx, &AuditLog{ActorEmail: "admin@example.com"})
	if err != nil {
		t.Fatalf("ClearAll() error = %v", err)
	}
	if deleted != 1 {
		t.Fatalf("ClearAll() deleted = %d, want 1", deleted)
	}

	logs, calls := repo.snapshot()
	if got, want := strings.Join(calls, ","), "batch,clear"; got != want {
		t.Fatalf("repository call order = %q, want %q", got, want)
	}
	if len(logs) != 1 || logs[0].Action != AuditActionAuditLogClear {
		t.Fatalf("logs after clear = %#v, want only the clear trace", logs)
	}
	if got := logs[0].Extra["deleted_rows"]; got != int64(1) {
		t.Fatalf("clear trace deleted_rows = %#v, want int64(1)", got)
	}
}

func TestAuditLogServiceClearAllDoesNotTruncateWhenPendingFlushFails(t *testing.T) {
	flushErr := errors.New("forced batch insert failure")
	repo := &auditLogMemoryRepository{batchInsertErr: flushErr}
	svc := NewAuditLogService(repo, nil)
	svc.Start()
	defer svc.Stop()
	...
	if err == nil || !strings.Contains(err.Error(), flushErr.Error()) {
		t.Fatalf("ClearAll() error = %v, want wrapped flush failure", err)
	}
	if deleted != 0 {
		t.Fatalf("ClearAll() deleted = %d, want 0", deleted)
	}

	logs, calls := repo.snapshot()
	if got, want := strings.Join(calls, ","), "batch"; got != want {
		t.Fatalf("repository calls = %q, want %q (no clear)", got, want)
	}
	if len(logs) != 0 {
		t.Fatalf("logs after failed flush = %#v, want no persisted records", logs)
	}

	// The failed batch must remain available for a later retry. Once persistence
	// recovers, a second clear flushes that original record before truncating it.
	repo.setBatchInsertError(nil)
	deleted, err = svc.ClearAll(ctx, &AuditLog{ActorEmail: "admin@example.com"})
	if err != nil {
		t.Fatalf("second ClearAll() error = %v", err)
	}
	if deleted != 1 {
		t.Fatalf("second ClearAll() deleted = %d, want 1", deleted)
	}
	logs, calls = repo.snapshot()
	if got, want := strings.Join(calls, ","), "batch,batch,clear"; got != want {
		t.Fatalf("repository calls after recovery = %q, want %q", got, want)
	}
	if len(logs) != 1 || logs[0].Action != AuditActionAuditLogClear {
		t.Fatalf("logs after recovered clear = %#v, want only the clear trace", logs)
	}
}
