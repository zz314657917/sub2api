...
func TestAssignSubscriptionRenewsExpiredSemanticMatch(t *testing.T) {
	...
	assignedAt := oldStart.Add(-time.Hour)
	assignedBy := int64(77)
	subRepo.seed(&UserSubscription{
		ID:                 12,
		UserID:             1002,
...
func TestAssignSubscriptionRenewsActiveSubscriptionPastExpiry(t *testing.T) {
	groupRepo := &subscriptionGroupRepoStub{
		group: &Group{ID: 1, SubscriptionType: SubscriptionTypeSubscription},
	}
	subRepo := newSubscriptionUserSubRepoStub()
	...
	sub, err := svc.AssignSubscription(context.Background(), &AssignSubscriptionInput{
		UserID:       1006,
		GroupID:      1,
		ValidityDays: MaxValidityDays + 1,
		Notes:        "expired by time",
	})
	after := time.Now()
