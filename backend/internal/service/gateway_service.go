...
type UpstreamFailoverError struct {
	...
	SameAccountRetryLimit       int           // 大于 0 时覆盖 handler 的默认同账号重试上限
	SameAccountRetryBackoffBase time.Duration // 大于 0 时按重试次数线性计算等待时长
}
...
func extractSystemTextAndCacheControl(system any) (string, any) {
	...
	case []any:
		...
		for _, item := range v {
			...
			if cacheControlValue, exists := block["cache_control"]; exists && cacheControlValue != nil {
				// The original system blocks collapse into one synthetic message block.
				// Keep the final caller-provided breakpoint as its closest equivalent.
...
func rewriteSystemForNonClaudeCode(body []byte, system any) []byte {
	...
	// 1. 提取原始 system prompt 文本及其最后一个缓存断点。
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	// 进入上面的来源覆盖：任意别名查无价会静默落 $0，含家族词的别名则可能被价格表的
	// 家族模糊匹配错计。除非管理员为别名显式配置了分组或渠道定价，composite 请求一律
	// 按实际转发的具体模型计费。
	...
	// 通用兜底：选定模型查不到任何价格时回退到实际转发的具体模型。已定价流量不受影响。
...
// 换成公开别名等非具体模型时，只有管理员为该名字显式配置了分组或渠道定价才按其计费，
// 否则回退到实际转发的具体模型，避免别名落入价格表的家族模糊匹配（错价）或查无价（$0）。
// 未发生来源覆盖时原样返回。
func (s *GatewayService) compositeBillableModel(ctx context.Context, apiKey *APIKey, billingModel, concreteBillingModel string) string {
	...
	logger.LegacyPrintf("service.gateway", "[Billing] composite billing model %q has no explicit pricing, billing by concrete model %q", billingModel, concreteBillingModel)
	return concreteBillingModel
}

// billableModelWithFallback 在选定计费模型（可能是 composite 公开别名或未定价的映射名）
// 查不到任何价格（显式分组/渠道价与全局价均无）时，按序回退到实际转发的具体模型，避免静默 $0 计费。
...
// hasResolvableTokenPricing 判断模型是否能在显式分组/渠道定价或全局价格表中解析出 token 价格。
...
// calculateTokenCost 计算 Token 计费：路径选择统一交给 BillingService，保证显式定价、
// Gemini 旧规则和分组长上下文开关使用相同的优先级。
func (s *GatewayService) calculateTokenCost(
	...
	if opts != nil && opts.LongContextThreshold > 0 {
		legacy = &LegacyLongContextRule{
			Threshold:  opts.LongContextThreshold,
			Multiplier: opts.LongContextMultiplier,
		}
	}
...
// LegacyLongContextRule exposes BillingService's platform rule to handlers.
...
func sanitizeCountTokensRequestBody(body []byte) []byte {
	...
	for _, path := range []string{
		...
		// OAuth mimicry may inject max_tokens for normal messages requests, but the
		// count_tokens endpoint rejects generation parameters.
		"max_tokens",
	} {
		if gjson.GetBytes(out, path).Exists() {
