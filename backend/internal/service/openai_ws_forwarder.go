...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	promptCacheKey := codexAccountIdentityOriginalPromptCacheKey(c, openAIWSPayloadString(payload, "prompt_cache_key"))
	...
	wsHeaders, sessionResolution := s.buildOpenAIWSHeaders(
		ctx,
		c,
		account,
		token,
	...
	for {
		...
		if eventType == "response.failed" {
			if hit, code, msg := detectOpenAICyberPolicy(message); hit {
				MarkOpsCyberPolicy(c, CyberPolicyMark{Code: code, Message: msg, Body: truncateString(string(message), 4096), UpstreamStatus: http.StatusOK, UpstreamInTok: usage.InputTokens, UpstreamOutTok: usage.OutputTokens})
			}
		}
...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	parseClientPayload := func(raw []byte) (openAIWSClientPayload, error) {
		...
		accountScoped, changed, scopeErr := applyCodexAccountIdentityClientMetadataRaw(normalized, codexAccountIdentitySource(c, account), getAPIKeyIDFromContext(c))
		if scopeErr != nil {
	...
	if s.shouldBridgeOpenAIWSHTTP(account, firstPayload.payloadBytes, firstPayload.previousResponseID) {
		...
		for turn := 1; ; turn++ {
			...
			result, bridgeErr := s.proxyOpenAIWSHTTPBridgeTurn(
				...
				currentBridgePayload.imageQuality,
	...
	acquireTurnLease := func(turn int, preferred string, forcePreferredConn bool) (*openAIWSConnLease, error) {
		...
		if acquireErr != nil {
			...
			if errors.As(acquireErr, &dialErr) && dialErr != nil && dialErr.StatusCode == http.StatusTooManyRequests {
				s.persistOpenAIWSRateLimitSignal(ctx, account, dialErr.ResponseHeaders, nil, "rate_limit_exceeded", "rate_limit_error", strings.TrimSpace(acquireErr.Error()), ingressSessionOriginalModel)
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string, imageBillingModel string, imageSizeTier string, imageQuality string, imageInputSize string) (*OpenAIForwardResult, error) {
		// An upstream lease can serve multiple client turns. Keep observation
		// scoped to this exact attempt so a failed/retried turn cannot leak its
		// reported model into the eventual successful result.
		responseModelObserver := &upstreamResponseModelObserver{}
		if lease == nil {
		...
		for {
			...
			// Observe the raw upstream event before restoring the client model or
			// applying tool/custom-namespace rewrites below.
	...
	for {
		...
		result, relayErr := sendAndRelay(turn, sessionLease, currentPayload, currentPayloadBytes, currentOriginalModel, currentImageBillingModel, currentImageSizeTier, currentImageQuality, currentImageInputSize)
		if relayErr != nil {
			lastTurnClean = false
