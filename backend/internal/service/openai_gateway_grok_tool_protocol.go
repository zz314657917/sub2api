...
// The local SSE bridge already emits one complete JSON document per data line;
// keep this small wrapper so the restorer can preserve non-data SSE fields.
type openAISSEJSONDocumentScanner struct{ scanner *bufio.Scanner }
...
func (s *openAISSEJSONDocumentScanner) Scan() bool   { return s != nil && s.scanner.Scan() }
func (s *openAISSEJSONDocumentScanner) Text() string { return s.scanner.Text() }
func (s *openAISSEJSONDocumentScanner) Err() error   { return s.scanner.Err() }
...
func adaptResponsesClientToolsForFunctionUpstreamWithMapping(
	...
	if err := decoder.Decode(&trailingValue); !errors.Is(err, io.EOF) {
		...
		return body, apicompat.ResponsesClientToolMapping{}, fmt.Errorf("decode %s Responses client tools trailing data: %w", upstream, err)
	}
