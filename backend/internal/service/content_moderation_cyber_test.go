...
func (r *contentModerationRuntimeSettingRepo) calls() (int, int) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func TestRecordCyberPolicyEvent_LogFailureStopsBeforeEmail(t *testing.T) {
	repo := &cyberOrderingTestRepo{createErr: errors.New("database unavailable")}
	emailSettings := &contentModerationRuntimeSettingRepo{values: map[string]string{}}
	emailService := NewEmailService(emailSettings, nil)
	svc := NewContentModerationService(
		&contentModerationTestSettingRepo{values: map[string]string{
			SettingKeyRiskControlEnabled: "true",
		}},
	...
	require.Equal(t, []string{"create"}, repo.snapshot())
	getValueCalls, getMultipleCalls := emailSettings.calls()
	require.Zero(t, getValueCalls)
	require.Zero(t, getMultipleCalls, "email settings must not be loaded when the audit row is not durable")
}
