...
func TestAccountTestService_TestAccountConnection_OpenAICompactOAuthSuccessPersistsSupport(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body:       io.NopCloser(strings.NewReader(`{"id":"resp_probe","output":[{"id":"cmp_probe","type":"compaction"}]}`)),
	}}
	...
	require.Equal(t, openai.CodexCLIOriginator+"/"+codexCLIVersion, upstream.lastReq.Header.Get("User-Agent"))
	require.Equal(t, openai.CodexCLIOriginator, upstream.lastReq.Header.Get("Originator"))
...
func TestAccountTestService_TestAccountConnection_OpenAIImageOAuthEnforcesFinalCodexIdentity(t *testing.T) {
	...
	require.Equal(t, "codex_cli_rs/0.142.0 (Ubuntu 22.4.0; x86_64) screen", upstream.lastReq.Header.Get("User-Agent"))
	require.Equal(t, "chatgpt-acc", upstream.lastReq.Header.Get("chatgpt-account-id"))
...
func TestAccountTestService_TestAccountConnection_OpenAICompactAPIKeyUsesNativeResponsesPath(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"id":"resp_probe_apikey","output":[{"id":"cmp_probe_apikey","type":"compaction"}]}`)),
	}}
	svc := &AccountTestService{
		accountRepo:  repo,
		httpUpstream: upstream,
...
func TestAccountTestService_TestAccountConnection_OpenAICompactAPIKeyDefaultBaseURLUsesV1Path(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"id":"resp_probe_apikey_default","output":[{"id":"cmp_probe_apikey_default","type":"compaction"}]}`)),
	}}
	svc := &AccountTestService{
		accountRepo:  repo,
		httpUpstream: upstream,
