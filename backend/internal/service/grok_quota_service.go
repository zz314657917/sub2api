...
func (s *GrokQuotaService) ProbeUsage(ctx context.Context, accountID int64) (*GrokQuotaProbeResult, error) {
	...
	if resp.StatusCode >= 400 {
		...
		return nil, infraerrors.Newf(mapGrokQuotaUpstreamStatus(resp.StatusCode), "GROK_QUOTA_PROBE_UPSTREAM_ERROR", "upstream returned %d for probe model %q: %s", resp.StatusCode, probeModel, bodyText)
	}
	return result, nil
}
...
func mapGrokQuotaUpstreamStatus(status int) int {
	switch {
