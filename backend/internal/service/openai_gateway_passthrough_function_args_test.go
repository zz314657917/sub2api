...
func TestForwardResponsesChatCompletionsFallbackKeepsFunctionArgumentsSingle(t *testing.T) {
	...
	upstreamBody := strings.Join([]string{
		...
		"",
		`data: {"id":"chatcmpl_tool","object":"chat.completion.chunk","model":"gpt-5.4","choices":[],"usage":{"prompt_tokens":3,"completion_tokens":2,"total_tokens":5}}`,
		"",
		"data: [DONE]",
		"",
	}, "\n")
	...
	completed := findSSEEvent(t, events, "response.completed", "")
	require.Equal(t, wantArgs, gjson.Get(completed, "response.output.0.arguments").String())
}
