...
func TestAntigravityGatewayService_ForwardGemini_FallbackReportsActualUpstreamModel(t *testing.T) {
	...
	c.Request = httptest.NewRequest(http.MethodPost,
		"/v1beta/models/gemini-primary:generateContent", bytes.NewReader(body))
	...
	account := &Account{
		ID: 9, Name: "acc-gemini-fallback", Platform: PlatformAntigravity,
	...
	result, err := svc.ForwardGemini(context.Background(), c, account,
		originalModel, "generateContent", true, body, false)
	require.NoError(t, err)
	require.NotNil(t, result)
