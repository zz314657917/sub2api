...
func (s *OpenAIQuotaService) accountRepository() AccountRepository {
	if s == nil {
		return nil
	}
	repo, _ := s.accountRepo.(AccountRepository)
	return repo
}
...
func (s *OpenAIQuotaService) prepareUpstreamCall(ctx context.Context, accountID int64) (accessToken, chatGPTAccountID, proxyURL string, fedRAMP bool, err error) {
	...
	if account.IsShadow() {
		resolved, rerr := resolveCredentialAccount(ctx, s.accountRepository(), account)
		if rerr != nil {
...
func (s *OpenAIQuotaService) recoverAgentIdentityTask(ctx context.Context, accountID int64, expectedTaskID string) error {
	...
	if account.IsShadow() {
		account, err = resolveCredentialAccount(ctx, s.accountRepository(), account)
		if err != nil || account == nil {
	...
	return ensureAgentIdentityTaskForAccount(ctx, s.accountRepository(), s.agentIdentityWS, &s.agentIdentityTaskMu, account, expectedTaskID)
}
...
func (s *OpenAIQuotaService) buildCodexQuotaHeaders(ctx context.Context, accountID int64, accessToken, chatGPTAccountID string, fedRAMP bool) (map[string]string, string, error) {
	...
	if account.IsShadow() {
		if resolved, resolveErr := resolveCredentialAccount(ctx, s.accountRepository(), account); resolveErr == nil && resolved != nil {
			account = resolved
	...
	if err := ensureAgentIdentityTaskForAccount(ctx, s.accountRepository(), s.agentIdentityWS, &s.agentIdentityTaskMu, account, ""); err != nil {
		return nil, "", err
	}
...
func (s *OpenAIQuotaService) redactQuotaErrorBody(ctx context.Context, accountID int64, body string) string {
	...
	return string(redactAgentIdentitySensitiveBodyForAccount(ctx, s.accountRepository(), account, []byte(body)))
}
...
// Compatibility wrappers retain the local helper names used by older quota
// tests and callers while sharing the upstream Codex header/status behavior.
func buildOpenAIQuotaCodexHeaders(accessToken, chatGPTAccountID string) map[string]string {
	return buildCodexCommonHeaders(accessToken, chatGPTAccountID, false)
}

func generateOpenAIQuotaRedeemRequestID() (string, error) {
	return generateRedeemRequestID()
}

func mapOpenAIQuotaUpstreamStatus(status int) int {
	return mapUpstreamStatus(status)
}

// Embedded here because the local topology does not contain the upstream split file.
