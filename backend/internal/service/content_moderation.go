...
func (s *ContentModerationService) validateConfig(ctx context.Context, cfg *ContentModerationConfig) error {
	...
	if cfg.ProxyID != nil {
		if s.proxyRepo == nil {
			return infraerrors.BadRequest("INVALID_CONTENT_MODERATION_PROXY", "代理服务器仓储不可用")
		}
...
// moderationHTTPClient returns the client selected for this moderation call.
// Proxy resolution or client construction errors are returned and never fall
...
func (s *ContentModerationService) resolveModerationProxyURL(ctx context.Context, proxyID int64) (string, error) {
	...
	if proxy == nil {
		return "", fmt.Errorf("resolve moderation proxy %d: empty proxy", proxyID)
	}
	proxyURL := proxy.URL()
