...
func TestCyberSessionBlockKey(t *testing.T) {
	...
	require.Empty(t, CyberSessionBlockKey(0, c1, b1), "non-positive API key IDs must fail open")
	require.Empty(t, CyberSessionBlockKey(-1, c1, b1), "negative API key IDs must fail open")
...
func TestRefreshCachedSettingsUpdatesCyberSessionBlockRuntimeImmediately(t *testing.T) {
	svc := &SettingService{}
	svc.cyberSessionBlockRuntimeCache.Store(&cachedCyberSessionBlockRuntime{
		enabled:   false,
		ttl:       time.Hour,
