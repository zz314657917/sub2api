...
func TestOpenAIStreamingContextWindowResponseFailedBeforeOutputPassesThrough(t *testing.T) {
	...
	resp := &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(strings.Join([]string{
			...
			"",
			"event: response.failed",
			`data: {"type":"response.failed","response":{"id":"resp_1","error":{"message":"Your input exceeds the context window of this model. Please adjust your input and try again.","code":null}}}`,
			"",
		}, "\n"))),
...
func TestOpenAIStreamingPassthroughResponseFailedBeforeOutputCapacityErrorRetriesSameAccount(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestOpenAIStreamingPassthroughResponseFailedBeforeOutputRateLimitReturns429(t *testing.T) {
	...
	resp := &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(strings.Join([]string{
			"event: response.failed",
			`data: {"type":"response.failed","response":{"id":"resp_rate_limited","error":{"code":"rate_limit_exceeded","message":"rate limited"}}}`,
			"",
		}, "\n"))),
...
func TestOpenAIGatewayServiceForwardGPT55InjectsModelSpecificInstructions(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-5.5","input":[{"type":"message","role":"user","content":"hello"}],"instructions":"   "}`)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body:       io.NopCloser(strings.NewReader(`{"id":"resp_gpt55_instructions","model":"gpt-5.5","status":"completed","output":[],"usage":{"input_tokens":1,"output_tokens":1}}`)),
	}}
...
func TestOpenAIGatewayServiceForwardNativeCompactionV2DoesNotUseRawChatFallback(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-5.4","stream":true,"input":[{"type":"message","role":"user","content":"hello"},{"type":"compaction_trigger"}]}`)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_native_compaction"}},
		Body: io.NopCloser(strings.NewReader(
			"data: {\"type\":\"response.output_item.done\",\"item\":{\"id\":\"cmp_1\",\"type\":\"compaction\"}}\n\n" +
				"data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_native_compaction\",\"output\":[{\"id\":\"cmp_1\",\"type\":\"compaction\"}],\"usage\":{\"input_tokens\":1,\"output_tokens\":1,\"total_tokens\":2}}}\n\n" +
				"data: [DONE]\n\n",
		)),
	}}
	svc := &OpenAIGatewayService{
...
func TestOpenAIGatewayServiceForwardAsAnthropicMappedNonCodexOmitsOriginator(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"claude-sonnet-4-20250514","max_tokens":64,"messages":[{"role":"user","content":"hello"}]}`)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(strings.Join([]string{
			...
			`data: {"type":"response.completed","response":{"id":"resp_bridge","model":"gpt-4o","usage":{"input_tokens":1,"output_tokens":1}}}`,
			"",
			"data: [DONE]",
			"",
		}, "\n"))),
	...
	require.Equal(t, "claude-cli/2.1.92 (external, cli)", upstream.lastReq.Header.Get("User-Agent"))
}

func TestOpenAIGatewayService_RecordLunaIdentityPairsOfficialCodexHeaders(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-5.6-luna","input":[{"type":"message","role":"user","content":"hello"}]}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
	c.Request.Header.Set("User-Agent", "cccc/0.100.0 (Mac OS X 14.0; arm64) iTerm (codex-tui; 0.100.0)")
	c.Request.Header.Set("Originator", "codex_cli_rs")
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"id":"resp_gpt56_luna","model":"gpt-5.6-luna","status":"completed","output":[],"usage":{"input_tokens":1,"output_tokens":1}}`)),
	}}
...
func newProxyStreamCircuitTestService() *OpenAIGatewayService {
...
func TestOpenAIStreamingPostOutputDisconnectQuarantinesSharedProxy(t *testing.T) {
	gin.SetMode(gin.TestMode)
	proxyID := int64(4698)
	account := &Account{ID: 469801, Platform: PlatformOpenAI, Type: AccountTypeOAuth, ProxyID: &proxyID}
	svc := newProxyStreamCircuitTestService()
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", nil)
	resp := &http.Response{
...
func TestOpenAIStreamingTerminalClearsProxyQuarantine(t *testing.T) {
	gin.SetMode(gin.TestMode)
	proxyID := int64(4699)
	account := &Account{ID: 469901, Platform: PlatformOpenAI, Type: AccountTypeOAuth, ProxyID: &proxyID}
	svc := newProxyStreamCircuitTestService()
...
func TestOpenAIStreamingClientCancellationDoesNotQuarantineProxy(t *testing.T) {
	gin.SetMode(gin.TestMode)
	proxyID := int64(4700)
	account := &Account{ID: 470001, Platform: PlatformOpenAI, Type: AccountTypeOAuth, ProxyID: &proxyID}
	svc := newProxyStreamCircuitTestService()
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	ctx, cancel := context.WithCancel(context.Background())
...
func TestOpenAIStreamingPassthroughPostOutputDisconnectQuarantinesSharedProxy(t *testing.T) {
	gin.SetMode(gin.TestMode)
	proxyID := int64(4701)
	account := &Account{ID: 470101, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, ProxyID: &proxyID}
	svc := newProxyStreamCircuitTestService()
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", nil)
