...
func TestForwardAsChatCompletions_BufferedCyberPolicyNoFailover(t *testing.T) {
	...
	require.Equal(t, http.StatusBadRequest, mark.UpstreamStatus, "ops status must match the buffered client response")
	require.True(t, c.Writer.Written(), "cyber error must be written to client (passthrough)")
	require.Equal(t, 1, strings.Count(rec.Body.String(), "flagged for cyber policy"), "handler must not append a second error body")
}
...
func TestForwardAsChatCompletions_StreamCyberPolicyDropsResult(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, mark.UpstreamStatus, "streaming client keeps the committed 200 status")
	require.Contains(t, rec.Body.String(), "data: [DONE]", "stream must terminate with [DONE]")
	require.Equal(t, 1, strings.Count(rec.Body.String(), "flagged for cyber policy"), "stream must contain exactly one cyber error event")
	require.Equal(t, 1, strings.Count(rec.Body.String(), "data: [DONE]"), "stream must contain exactly one terminal marker")
}
...
func TestForwardCompatJSONCyberPolicyNoFailover(t *testing.T) {
	for _, tc := range []struct {
		name      string
		anthropic bool
	}{
		{name: "chat_completions", anthropic: false},
		{name: "anthropic_messages", anthropic: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			cyberBody := `{"error":{"code":"cyber_policy","message":"blocked by cyber policy"}}`
			...
			require.Equal(t, 1, strings.Count(rec.Body.String(), "blocked by cyber policy"))
			mark := GetOpsCyberPolicy(c)
