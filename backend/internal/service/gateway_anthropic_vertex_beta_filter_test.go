...
func TestVertexBetaFilter_StripsUnsupportedClaudeCodeTokens(t *testing.T) {
	...
	for _, bad := range []string{
		...
		require.False(t, anthropicBetaTokensContains(outBeta, bad), "token %q should be stripped, beta=%q", bad, outBeta)
	}
	...
	for _, keep := range []string{
		...
		require.True(t, anthropicBetaTokensContains(outBeta, keep), "token %q should be preserved, beta=%q", keep, outBeta)
	}
}

func TestVertexBetaFilter_DropsHeaderWhenAllUnsupported(t *testing.T) {
	c := newVertexBetaTestContext(t, "prompt-caching-scope-2026-01-05,redact-thinking-2026-02-12")
	body := []byte(`{"model":"claude-opus-4-7","max_tokens":32,"messages":[{"role":"user","content":"hi"}]}`)
...
func TestVertexBetaFilter_BlocksViaBetaPolicy(t *testing.T) {
	...
	c := newVertexBetaTestContext(t, "interleaved-thinking-2025-05-14,context-management-2025-06-27")
	body := []byte(`{"model":"claude-opus-4-7","max_tokens":32,"messages":[{"role":"user","content":"hi"}]}`)
...
func TestFilterVertexBetaTokens(t *testing.T) {
	...
	t.Run("dedupe", func(t *testing.T) {
		out := filterVertexBetaTokens("context-1m-2025-08-07,context-1m-2025-08-07", nil)
