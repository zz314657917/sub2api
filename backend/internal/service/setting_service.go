...
// OmittedSettingKeys marks settings absent from a partial admin update. The
// settings DTO uses value fields, so omitted values otherwise become
// indistinguishable from explicit zero-value clears after JSON binding.
...
// LoadForwardedClientIPSettings loads the persisted security-IP settings into
// the config runtime snapshot. Missing values fall back to the file config;
// malformed persisted headers fail closed and are never auto-enabled.
func (s *SettingService) LoadForwardedClientIPSettings(ctx context.Context) error {
	...
	headers := append([]string{}, runtimeSettings.Headers...)
...
// LoadAPIKeyACLTrustForwardedIPSetting retains the historical entry point for
// callers while loading both the toggle and its custom header list.
...
// UpdateSettingsOmitting persists settings while leaving omitted keys at their
// stored values. A nil omission set preserves whole-document behavior.
...
// auth-source defaults while leaving omitted value-typed settings unchanged.
...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	updates[SettingKeyAuditLogRetentionDays] = strconv.Itoa(normalizeAuditLogRetentionDays(settings.AuditLogRetentionDays))
	...
	if settings.AffiliateAPICallRewardAmount < 0 || math.IsNaN(settings.AffiliateAPICallRewardAmount) || math.IsInf(settings.AffiliateAPICallRewardAmount, 0) {
	...
	if settings.WelfareFirstRechargeBonusValidDays < 0 {
		settings.WelfareFirstRechargeBonusValidDays = 0
	}
	...
	updates[SettingKeyWelfareFirstRechargeBonusValidDays] = strconv.Itoa(settings.WelfareFirstRechargeBonusValidDays)
	updates[SettingKeyWelfareFirstRechargeBonusStackMonthly] = strconv.FormatBool(settings.WelfareFirstRechargeBonusStackMonthly)
...
// GetAffiliateAPICallRewardAmount 返回被邀请人首次调用 API 后可领取的固定返利金额。
// 返回 0 表示该奖励未配置或关闭。
func (s *SettingService) GetAffiliateAPICallRewardAmount(ctx context.Context) float64 {
	if s == nil || s.settingRepo == nil {
		return AffiliateAPICallRewardAmountDefault
	}
	raw, err := s.settingRepo.GetValue(ctx, SettingKeyAffiliateAPICallRewardAmount)
	if err != nil {
		return AffiliateAPICallRewardAmountDefault
	}
...
// disable a valid relying party, never replace or weaken it.
...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	forwardedClientIPTrust := false
	forwardedClientIPHeaders := []string{}
	if s != nil && s.cfg != nil {
		runtimeSettings := s.cfg.ForwardedClientIPSettings()
		if normalized, normalizeErr := config.NormalizeForwardedClientIPHeaders(runtimeSettings.Headers); normalizeErr == nil {
			forwardedClientIPTrust = runtimeSettings.TrustForwardedIP
			forwardedClientIPHeaders = normalized
		}
	}
	...
	defaults := map[string]string{
		...
		SettingKeyAPIKeyACLTrustForwardedIP:                strconv.FormatBool(forwardedClientIPTrust),
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	if s != nil && s.cfg != nil {
		runtimeSettings := s.cfg.ForwardedClientIPSettings()
		forwardedClientIPTrust = runtimeSettings.TrustForwardedIP
		forwardedClientIPHeaders = append(forwardedClientIPHeaders, runtimeSettings.Headers...)
	}
	if value, ok := settings[SettingKeyAPIKeyACLTrustForwardedIP]; ok {
		forwardedClientIPTrust = strings.TrimSpace(value) == "true"
	}
	if value, ok := settings[SettingKeyForwardedClientIPHeaders]; ok {
		parsed, err := parseForwardedClientIPHeadersSetting(value)
		if err != nil {
			// A malformed persisted source must never re-enable raw forwarded trust.
			slog.Warn("invalid persisted forwarded client IP headers; disabling forwarded trust", "error", err)
			forwardedClientIPTrust = false
			forwardedClientIPHeaders = []string{}
		} else {
	...
	result := &SystemSettings{
		...
		APIKeyACLTrustForwardedIP:        forwardedClientIPTrust,
	...
	if apiCallRewardAmount, err := strconv.ParseFloat(settings[SettingKeyAffiliateAPICallRewardAmount], 64); err == nil &&
		apiCallRewardAmount >= 0 &&
		!math.IsNaN(apiCallRewardAmount) &&
		!math.IsInf(apiCallRewardAmount, 0) {
		result.AffiliateAPICallRewardAmount = apiCallRewardAmount
	} else {
	...
	result.WelfareFirstRechargeBonusValidDays = parseNonNegativeIntSetting(settings[SettingKeyWelfareFirstRechargeBonusValidDays], 0)
	result.WelfareFirstRechargeBonusStackMonthly = settings[SettingKeyWelfareFirstRechargeBonusStackMonthly] == "true"
...
// GetOpenAIQuotaAutoPauseSettings returns cached global defaults for the OpenAI
// account quota auto-pause hot path. It never blocks on DB reads; stale or empty
// cache triggers a background refresh and returns the last known value.
...
// SetOpenAIQuotaAutoPauseSettings makes a persisted settings update visible to
// the scheduling hot path immediately.
