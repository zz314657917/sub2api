...
func TestOpenAIGatewayService_OAuthMessagesBridgeDoesNotInjectDefaultInstructions(t *testing.T) {
	...
	require.NotContains(t, upstream.lastReq.Header, "Originator")
}
...
func TestOpenAIGatewayService_OpenAIPassthrough_Capacity400RetriesBeforeFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
		...
		t.Run(accountType, func(t *testing.T) {
			...
			upstream := &httpUpstreamRecorder{resp: &http.Response{
				...
				Header: http.Header{
					...
					"x-request-id": []string{"rid-capacity-passthrough"},
				},
