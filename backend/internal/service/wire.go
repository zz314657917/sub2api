...
func ProvidePelicanTestService(repo PelicanTestRepository, accounts AccountRepository, groups GroupRepository, tester *AccountTestService, settings SettingRepository, gateway *OpenAIGatewayService, concurrency *ConcurrencyService) *PelicanTestService {
	svc := NewPelicanTestService(repo, accounts, groups, tester)
	svc.SetAccountSelector(pelicanMixedSelector{gateway: gateway, accounts: accounts, groups: groups, concurrency: concurrency})
...
func ProvideAuthService(
	...
	svc.SetInitialAPIKeyBootstrapper(apiKeyService)
	return svc
}
...
func ProvideSettingService(settingRepo SettingRepository, groupRepo GroupRepository, proxyRepo ProxyRepository, cfg *config.Config) *SettingService {
	...
	svc.SetStudioBridgeDefaultGroupReader(groupRepo)
...
func ProvideAdminService(
	...
	if setter, ok := svc.(interface {
		SetChannelCacheInvalidator(channelCacheInvalidator)
	}); ok {
		setter.SetChannelCacheInvalidator(channelService)
	}
	return svc
}
...
var ProviderSet = wire.NewSet(
	...
	NewGroupService,
	ProvideCompositeRouteAdminService,
	NewAccountService,
	...
	NewPromoService,
	ProvideUsageService,
	ProvideLeaderboardLotteryRunner,
	NewDashboardService,
	...
	ProvideAffiliateRiskScannerService,
	...
	ProvideUsageBillingSettlementService,
