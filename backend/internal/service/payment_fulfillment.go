...
func (s *PaymentService) ExecuteGroupBuyFulfillment(ctx context.Context, oid int64) error {
	...
	if s.groupBuySvc == nil {
		return infraerrors.InternalServer("GROUP_BUY_FULFILLMENT_NOT_CONFIGURED", "group buy fulfillment is not configured")
	}
	c, err := s.entClient.PaymentOrder.Update().
		...
		Save(ctx)
	if err != nil {
		return fmt.Errorf("lock group buy fulfillment: %w", err)
	}
	if c == 0 {
		return nil
	}
	if err := s.groupBuySvc.HandleGroupBuyOrderPaid(ctx, oid); err != nil {
		s.markFailed(ctx, oid, err)
		return err
	}
	if err := s.markCompleted(ctx, o, "GROUP_BUY_SEAT_PAID"); err != nil {
		s.markFailed(ctx, oid, err)
		return err
	}
	return nil
...
func (s *PaymentService) applyAffiliateRebateForOrder(ctx context.Context, o *dbent.PaymentOrder) error {
	...
	// Package credit includes promotional points; only the original package
	// price earns a rebate. Use the order snapshot, not current package settings
	// or PayAmount (which can include a payment processing fee).
	baseAmount := o.Amount
	if packagePayAmount := floatSnapshotValue(o.ProviderSnapshot, "recharge_package_pay_amount"); isValidProviderAmount(packagePayAmount) {
		baseAmount = packagePayAmount
	}
