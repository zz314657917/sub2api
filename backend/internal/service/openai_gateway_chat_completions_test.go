...
func TestForwardAsChatCompletions_EnforcesCodexCLIOnlyRestriction(t *testing.T) {
	...
	c.Request.Header.Set("originator", "my_client")

	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"id":"unused"}`)),
	}}
	svc := &OpenAIGatewayService{
		cfg:          &config.Config{},
		httpUpstream: upstream,
...
func TestForwardAsChatCompletions_TransportErrorReturnsFailover(t *testing.T) {
	...
	svc := &OpenAIGatewayService{
		cfg:          &config.Config{},
		httpUpstream: &httpUpstreamRecorder{err: errors.New("dial tcp: connection refused")},
	}
	account := &Account{
		ID:          12,
		Name:        "openai-oauth",
...
func TestForwardAsChatCompletions_BufferedTransientResponseFailedTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstreamBody := strings.Join([]string{
		`event: response.failed`,
		`data: {"type":"response.failed","response":{"id":"resp_failed","object":"response","model":"gpt-5.5","status":"failed","output":[],"error":{"code":"server_is_overloaded","message":"overloaded"}}}`,
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_chat_failed_buffered_transient"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
