...
func TestRedactCredentials_StripsSensitiveKeysAndReportsStatus(t *testing.T) {
	in := map[string]any{
		...
		"agentPrivateKey":       "agent-camel-secret",
		"agentprivatekey":       "agent-lower-secret",
		// 非敏感
	...
	require.NotContains(t, out, "agentPrivateKey")
	require.NotContains(t, out, "agentprivatekey")
	...
	require.True(t, status["has_agentPrivateKey"])
	require.True(t, status["has_agentprivatekey"])
...
func TestRedactCredentials_AllKnownSensitiveKeys(t *testing.T) {
	keys := []string{
		...
		"agent_private_key", "agentPrivateKey", "agentprivatekey",
	}
