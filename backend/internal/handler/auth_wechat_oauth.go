...
func (h *AuthHandler) CompleteWeChatOAuthRegistration(c *gin.Context) {
	...
	isNewUser, err := isOAuthEmailNewUser(c.Request.Context(), h.entClient(), email)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	...
	tokenPair, user, err := h.authService.LoginOrRegisterOAuthWithTokenPair(ctx, email, username, req.InvitationCode, req.AffCode)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
