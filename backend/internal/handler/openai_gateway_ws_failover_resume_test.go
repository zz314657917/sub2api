...
func TestOpenAIWSNextAttemptMessageKeepsCurrentAttemptForInitialFailover(t *testing.T) {
	current := []byte(`{"type":"response.create","input":"first"}`)
	...
	next[0] = 'x'
	require.Equal(t, byte('{'), current[0], "attempt payload must be cloned")
}
