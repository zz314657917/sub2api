...
// SetNotificationEmailService attaches template administration without changing
// the constructor signature used by existing handler tests.
...
func (h *SettingHandler) GetSettings(c *gin.Context) {
	...
	payload := dto.SystemSettings{
		...
		ForwardedClientIPHeaders:               append([]string{}, settings.ForwardedClientIPHeaders...),
...
// persisted setting key. Other value-typed UpdateSettingsRequest fields use the
// same name for both.
var settingKeyJSONAliases = map[string]string{
	"smtp_from_email": service.SettingKeySMTPFrom,
}

// settingKeyByJSONName maps value-typed top-level request fields to their
// persisted setting keys. Pointer fields already retain omission semantics in
// the existing handler merge logic, so only value fields need presence tracking.
...
func buildSettingKeyByJSONName() map[string]string {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if settingKey, ok := settingKeyJSONAliases[jsonName]; ok {
			keys[jsonName] = settingKey
			continue
		}
		keys[jsonName] = jsonName
	}
	return keys
}
...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	affiliateAPICallRewardAmount := previousSettings.AffiliateAPICallRewardAmount
	if req.AffiliateAPICallRewardAmount != nil {
		affiliateAPICallRewardAmount = *req.AffiliateAPICallRewardAmount
	}
	...
	forwardedClientIPHeaders := append([]string{}, previousSettings.ForwardedClientIPHeaders...)
	if req.ForwardedClientIPHeaders != nil {
		forwardedClientIPHeaders = append([]string{}, (*req.ForwardedClientIPHeaders)...)
	}
	...
	settings := &service.SystemSettings{
		...
		APIKeyACLTrustForwardedIP:        boolValueOrDefault(req.APIKeyACLTrustForwardedIP, previousSettings.APIKeyACLTrustForwardedIP),
	...
	payload := dto.SystemSettings{
		...
		ForwardedClientIPHeaders:               append([]string{}, updatedSettings.ForwardedClientIPHeaders...),
...
func diffSettings(before *service.SystemSettings, after *service.SystemSettings, beforeAuthSourceDefaults *service.AuthSourceDefaultSettings, afterAuthSourceDefaults *service.AuthSourceDefaultSettings, req UpdateSettingsRequest) []string {
	...
	if before.APIKeyACLTrustForwardedIP != after.APIKeyACLTrustForwardedIP {
		changed = append(changed, service.SettingKeyAPIKeyACLTrustForwardedIP)
	}
	if !equalStringSlice(before.ForwardedClientIPHeaders, after.ForwardedClientIPHeaders) {
		changed = append(changed, service.SettingKeyForwardedClientIPHeaders)
	}
...
func studioBridgeSettingsToDTO(settings service.StudioBridgeAppSettings) dto.StudioBridgeAppSettings {
	return dto.StudioBridgeAppSettings{
		...
		DefaultFallbackGroup: settings.DefaultFallbackGroup,
...
func studioBridgeSettingsFromDTO(settings dto.StudioBridgeAppSettings) service.StudioBridgeAppSettings {
	return service.StudioBridgeAppSettings{
		...
		DefaultFallbackGroup: strings.TrimSpace(settings.DefaultFallbackGroup),
		DefaultAPIRoutes:     settings.DefaultAPIRoutes,
