...
// ApplyOAuthCredentials persists OAuth re-authorization credentials without replacing Extra.
...
// RefreshQuota handles refreshing expired account quota windows.
// POST /api/v1/admin/accounts/:id/refresh-quota
func (h *AccountHandler) RefreshQuota(c *gin.Context) {
	...
	account, err := h.adminService.RefreshAccountQuota(c.Request.Context(), accountID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *AccountHandler) GetAvailableModels(c *gin.Context) {
	...
	if account.IsGemini() {
		...
		// channel cannot serve unless the administrator explicitly configured
		// a model mapping for this account.
		if account.IsOAuth() {
		...
		// API Key accounts and Google One accounts with an explicit mapping
		// expose the configured request model names.
		mapping := account.GetModelMapping()
		if len(mapping) == 0 {
			response.Success(c, geminicli.DefaultModels)
			return
