...
func TestDashboardTrendCacheSeparatesUpstreamModelMismatchTriState(t *testing.T) {
	...
	_, hit, err := handler.getUsageTrendCached(context.Background(), start, end, "day", 0, 0, 0, 0, "", nil, nil, nil, nil, nil)
	require.NoError(t, err)
	require.False(t, hit)
	_, hit, err = handler.getUsageTrendCached(context.Background(), start, end, "day", 0, 0, 0, 0, "", nil, nil, nil, nil, &trueValue)
	require.NoError(t, err)
	require.False(t, hit)
	_, hit, err = handler.getUsageTrendCached(context.Background(), start, end, "day", 0, 0, 0, 0, "", nil, nil, nil, nil, &falseValue)
	require.NoError(t, err)
	require.False(t, hit)
	_, hit, err = handler.getUsageTrendCached(context.Background(), start, end, "day", 0, 0, 0, 0, "", nil, nil, nil, nil, &trueValue)
	require.NoError(t, err)
