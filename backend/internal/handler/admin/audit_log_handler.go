...
func (h *AuditLogHandler) Clear(c *gin.Context) {
	if h == nil || h.auditService == nil || h.totpService == nil {
		response.ErrorWithDetails(c, http.StatusServiceUnavailable,
			"Audit log clearing service is unavailable", "AUDIT_LOG_CLEAR_UNAVAILABLE", nil)
		return
	}
