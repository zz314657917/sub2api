...
// when recordStandaloneUsage=false the function still sets the guard flag (mark present),
...
func TestShouldRecordStandaloneCyberUsageAvoidsPartialUsageDuplicate(t *testing.T) {
	err := errors.New("upstream response failed")
	require.True(t, shouldRecordStandaloneCyberUsage(err, nil), "nil result needs the standalone cyber usage path")
	require.False(t, shouldRecordStandaloneCyberUsage(err, &service.OpenAIForwardResult{}), "partial result is recorded by the normal partial usage path")
	require.False(t, shouldRecordStandaloneCyberUsage(nil, &service.OpenAIForwardResult{}), "successful result uses the normal usage path")
}
...
func TestBuildCyberPolicyOpsErrorEntry_RedactsSensitivePayload(t *testing.T) {
	secret := "sk-proj-abcdefghijklmnopqrstuvwxyz1234567890"
	entry := buildCyberPolicyOpsErrorEntry(cyberPolicyOpsErrorMeta{}, &service.CyberPolicyMark{
		Message: "blocked " + secret,
		Body:    `{"error":{"code":"cyber_policy","token":"` + secret + `"}}`,
	})

	require.NotNil(t, entry)
	require.NotContains(t, entry.ErrorMessage, secret)
	require.NotContains(t, entry.ErrorBody, secret)
}
