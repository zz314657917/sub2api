...
func eventColumns(alias string) string {
	return fmt.Sprintf(`%[1]s.id,%[1]s.job_id,%[1]s.request_id,%[1]s.user_id,%[1]s.username_snapshot,
		...
		%[1]s.guard_endpoint_id,%[1]s.policy_id,%[1]s.policy_version,%[1]s.matched_rule_id,%[1]s.owasp_tags,%[1]s.config_version,
...
func scanEvent(row rowScanner, withFullPrompt bool) (*Event, error) {
	...
	var categories, matched, scores, evidence, owasp []byte
	dest := []any{&event.ID, &event.JobID, &event.Snapshot.RequestID, &userID,
		...
		&event.ScannerVersion, &event.GuardEndpointID, &event.PolicyID, &event.PolicyVersion, &event.MatchedRuleID, &owasp,
	...
	_ = json.Unmarshal(owasp, &event.OWASPTags)
