...
// ExtractBlockingPromptSnapshot optionally narrows synchronous checks to the
// latest client turn and the nearest preceding model output. Async auditing
// continues to retain the complete client-controlled transcript in Redis.
...
// only when an administrator opts in. The async worker always scans the full
// client-controlled transcript.
