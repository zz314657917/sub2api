...
func TestConfigManagerMalformedColdStartFailsClosedWhenRiskControlIsEnabled(t *testing.T) {
	manager := &ConfigManager{}
	manager.observeExpectedState(`{"enabled":true`, true)
...
// config_loaded must stay a change signal instead of repeated heartbeat logs,
// while still reporting the first load, real config changes and recovery.
func TestConfigLoadedIsLoggedOnlyWhenSomethingChanged(t *testing.T) {
	...
	require.NoError(t, manager.Reload(context.Background()))
	require.NoError(t, manager.Reload(context.Background()))
	require.Equal(t, 1, loadedCount(), "unchanged refreshes must stay silent")

	repository.values[SettingKeyRiskControl] = "true"
	require.NoError(t, manager.Reload(context.Background()))
	require.Equal(t, 2, loadedCount(), "risk control changes must be logged")
	...
	require.NoError(t, manager.Reload(context.Background()))
	require.Equal(t, 3, loadedCount(), "config version changes must be logged")
	...
	require.NoError(t, manager.Reload(context.Background()))
	require.Equal(t, 4, loadedCount(), "recovery from a failed reload must be visible")
}
