...
func (m *AtomicMetrics) Snapshot() GuardMetricsSnapshot {
	...
	snapshot := GuardMetricsSnapshot{
		...
		ShadowRuns: m.shadowRuns.Load(), ShadowChanges: m.shadowChanges.Load(),
	}
...
func (m *AtomicMetrics) IncShadow(changed bool) {
	if m == nil {
		return
	}
	m.shadowRuns.Add(1)
	if changed {
		m.shadowChanges.Add(1)
	}
}
