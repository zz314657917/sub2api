...
func promptAdminRouter(service PromptAdminService) *gin.Engine {
	...
	group.POST("/policy/preview", handler.PreviewPolicy)
	group.POST("/policy/shadow", handler.ShadowPolicy)
	group.PUT("/policy/draft", handler.SavePolicyDraft)
	group.POST("/policy/publish", handler.PublishPolicyDraft)
	group.POST("/policy/rollback", handler.RollbackPolicy)
