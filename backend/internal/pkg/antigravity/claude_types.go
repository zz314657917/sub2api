...
var claudeModels = []modelDef{
	...
	{ID: "claude-opus-4-8", DisplayName: "Claude Opus 4.8", CreatedAt: "2026-05-28T00:00:00Z"},
...
// IsGeminiReasoningModel 判断是否为不支持部分生成参数和强制 ToolConfig 的 Gemini 推理模型。
func IsGeminiReasoningModel(modelID string) bool {
	lowerID := strings.ToLower(modelID)
	for _, m := range geminiModels {
		if m.IsReasoning && strings.Contains(lowerID, m.ID) {
			return true
		}
	}
	return false
