...
// CompositeModelRoute holds the administrator-owned model aliases for a composite group.
...
func (CompositeModelRoute) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("upstream_model").
			MaxLen(200).
			Default("").
			Comment("Provider model identifier; empty keeps the requested model."),
		field.String("endpoint").
			MaxLen(50).
			Default("any").
			Comment("Endpoint scope such as any, messages, responses, or chat_completions."),
		field.Int("priority").
			Default(100).
			Comment("Lower values sort first within the same candidate set."),
		field.Bool("enabled").
			Default(true),
		field.String("notes").
			Optional().
