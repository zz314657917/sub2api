...
describe('UpstreamBillingRateCell', () => {
  ...
  it('shows the upstream usage balance below the rate and in the probe details', async () => {
    const wrapper = mount(UpstreamBillingRateCell, {
        ...
        account: makeAccount({
          extra: {
            upstream_billing_probe: {
              ...
              data: {
                ...billingData,
                upstream_balance: { status: 'ok', kind: 'wallet', amount: 12.3456789, unit: 'USD' }
              },
              received_at: '2026-07-13T00:00:00Z',
              fresh_until: '2026-07-14T00:00:00Z',
              last_attempt_at: '2026-07-13T00:00:00Z',
    ...
    expect(wrapper.get('[data-testid="upstream-billing-balance-inline"]').text()).toBe(
      'admin.accounts.upstreamBilling.upstreamBalanceInline:12.35,USD'
    )
    ...
    expect(tooltip.querySelector('[data-testid="upstream-billing-balance"]')?.textContent).toContain(
      'admin.accounts.upstreamBilling.upstreamBalance:admin.accounts.upstreamBilling.upstreamBalanceWallet,12.35,USD'
    )
    wrapper.unmount()
  })

  it('labels an unlimited upstream subscription without rendering a negative balance', async () => {
    const wrapper = mount(UpstreamBillingRateCell, {
      attachTo: document.body,
      props: {
        account: makeAccount({
          extra: {
            upstream_billing_probe: {
              ...
              data: {
                ...billingData,
                upstream_balance: { status: 'ok', kind: 'subscription', unlimited: true, unit: 'USD' }
              },
              received_at: '2026-07-13T00:00:00Z',
              fresh_until: '2026-07-14T00:00:00Z',
              last_attempt_at: '2026-07-13T00:00:00Z',
    ...
    expect(wrapper.get('[data-testid="upstream-billing-balance-inline"]').text()).toBe(
      'admin.accounts.upstreamBilling.upstreamBalanceInlineUnlimited'
    )
    ...
    const tooltip = tooltips[tooltips.length - 1] as HTMLElement
    expect(tooltip.querySelector('[data-testid="upstream-billing-balance"]')?.textContent).toContain(
      'admin.accounts.upstreamBilling.upstreamBalanceUnlimited:admin.accounts.upstreamBilling.upstreamBalanceSubscription'
    )
    expect(tooltip.textContent).not.toContain('-1')
    wrapper.unmount()
  })
