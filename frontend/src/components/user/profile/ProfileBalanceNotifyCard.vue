      ...
      <template v-if="notifyEnabled">
        ...
        <div>
          ...
          <div class="flex items-center gap-2">
            <span class="text-gray-500">{{ CREDIT_SYMBOL }}</span>
            <input
              ...
              step="0.01"
              class="input flex-1"
              :placeholder="systemDefaultThreshold > 0 ? `${t('profile.balanceNotify.systemDefault')} ${formatCreditAmount(systemDefaultThreshold)}` : t('profile.balanceNotify.thresholdPlaceholder')"
            />
            <button
