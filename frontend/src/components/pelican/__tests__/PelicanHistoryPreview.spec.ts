...
describe('history animated previews', () => {
  ...
  it('keeps the preview frame natural-height instead of clipping it to 240px', async () => {
    api.result.mockResolvedValue({ status: 'success', html: `<main>${'long content '.repeat(150)}</main>` })
    const wrapper = mount(PelicanHistoryPreview, { props: { resultId: 14 } })
    await flushPromises()
    expect(wrapper.get('.preview').attributes('style')).toBeUndefined()
    expect(wrapper.get('iframe').attributes('style')).toContain('height:')
    wrapper.unmount()
  })
  it('uses a viewport-bounded scrollable frame for interactive detail', () => {
    const wrapper = mount(PelicanSandboxFrame, { props: { html: '<p>long detail</p>', replayKey: 0, interactive: true } })
    const frame = wrapper.get('iframe')
    expect(frame.classes()).toContain('pelican-frame--interactive')
