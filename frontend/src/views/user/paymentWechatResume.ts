...
export function parseWechatResumeRoute(
  ...
  const rechargePackageId = readQueryString(query, 'recharge_package_id').trim()
  ...
  if (wechatResumeToken) {
    return {
      ...
      rechargePackageId: rechargePackageId || undefined,
    }
  }
...
export function stripWechatResumeQuery(query: LocationQuery): LocationQueryRaw {
  ...
  delete nextQuery.recharge_package_id
  return nextQuery
}
