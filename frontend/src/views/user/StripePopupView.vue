<template>
  <div class="flex min-h-screen items-center justify-center bg-[#faf9f5] p-4 dark:bg-[#141413]">
    <div
      class="w-full max-w-md space-y-4 rounded-lg border border-[#d8cec2] bg-[#fffaf5] p-6 shadow-lg dark:border-dark-700 dark:bg-dark-900"
    >
      <!-- Amount + Order ID -->
      ...
      <div v-if="error" class="space-y-3">
        ...
        <button
          class="w-full text-sm underline underline-offset-2 transition hover:opacity-80"
      ...
      <div v-else-if="success" class="space-y-3 py-4 text-center">
        <div class="text-5xl text-[#5f7f68] dark:text-[#9ab3a0]">✓</div>
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('payment.result.success') }}</p>
        <button
          class="text-sm underline underline-offset-2 transition hover:opacity-80"
          :style="{ color: methodColor }"
          @click="closeWindow"
        >
          {{ t('common.close') }}
