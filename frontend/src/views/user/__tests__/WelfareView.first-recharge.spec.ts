...
describe('WelfareView new-user first recharge gift card', () => {
  it('renders the first recharge gift card and links to the recharge page', () => {
    ...
    expect(welfareView).toContain('first_bonus_claimable')
    expect(welfareView).toContain('claimWelfareFirstRechargeBonus')
    expect(welfareView).toContain('updateRecharge')
