...
interface SelectableUser extends SimpleUser {
  deleted?: boolean;
}
...
const searchResults = ref<SelectableUser[]>([]);
const searchLoading = ref(false);
const showDropdown = ref(false);
const selectedUsers = ref<Record<number, SelectableUser>>({});
  ...
  const users = await Promise.all(
    missing.map(async (id) => {
      try {
        ...
        return { id: user.id, email: user.email } satisfies SelectableUser;
      } catch {
        return null;
      }
    }),
