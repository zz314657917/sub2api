  ...
  it('keeps non-critical events redacted even when a legacy full_prompt value is present', async () => {
    const event: PromptAuditEvent = {
      ...
      snapshot: {
        ...
        endpoint: '/v1/chat/completions', protocol: 'openai_chat', model: 'gpt-test',
        prompt_hash: 'b'.repeat(64), redacted_preview: 'legacy redacted preview', full_prompt: 'legacy raw prompt', prompt_length: 20,
        message_count: 1, stage: 'http',
      },
    }
    const wrapper = mount(EventDetailDialog, {
    ...
    expect(wrapper.get('[data-test="risk-prompt-full"]').text()).not.toContain('legacy raw prompt')
  })
})
