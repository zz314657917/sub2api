export default {
  ...
  openWebUI: {
    title: 'Open WebUI 兼容入口',
    subtitle: '旧版 Open WebUI 跳转能力已默认关闭，当前主入口请使用站内聊天生图。',
    apiKey: 'API Key',
    loadingKeys: '正在加载可用 API Key',
    selectKey: '选择 API Key',
    noKeysTitle: '还没有可用 API Key',
    noKeysHint: 'Open WebUI 兼容入口需要可用的 active API Key，当前推荐使用站内聊天生图。',
    createKey: '创建 API Key',
    keyPrivacyHint: '真实 API Key 只会在服务端兑换并绑定到 Open WebUI 会话，不会出现在浏览器 URL 中。',
    active: '可用',
    imageReady: '支持生图',
    group: '分组',
    platform: '平台',
    ...
    neverExpires: '永久有效',
    unlimited: '不限额度',
    launchPanelTitle: '打开 Open WebUI',
    launchPanelHint: '仅在后台显式启用 Open WebUI 集成后可用。默认情况下请使用站内聊天生图。',
    launch: '打开 Open WebUI',
    opening: '正在打开',
    loadKeysFailed: '加载 API Key 失败',
    popupBlocked: '新标签页被浏览器拦截，请允许弹窗后重试',
    launchFailed: '打开 Open WebUI 失败'
  },

  chatImageStudio: {
    title: '聊天生图',
    subtitle: '在 Sub2API 内完成聊天、作画、图库和任务队列',
