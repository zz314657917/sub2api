export default {
    ...
    seconds: '秒',
    firstResponseTimeoutEnable: '超时自动切组',
    firstResponseTimeoutBulkSeconds: '统一首响应超时秒数',
    firstResponseTimeoutApplyAll: '应用到全部',
    firstResponseTimeoutPrefix: '无响应',
    firstResponseTimeoutSuffix: '秒切下一组',
    firstResponseTimeoutDisabled: '秒（默认）',
    firstResponseTimeoutRouteSeconds: '该分组等待秒数。0 使用默认规则：首次选中时不自动切组，后续选中时沿用首选分组的等待时间',
    firstResponseTimeoutInvalid: '请输入 1–600 秒的整数；单组可填 0，含义见“切组规则”',
    firstResponseTimeoutHint: '超时未收到有效响应，自动尝试下一可用分组。各组可单独设置等待时间。',
    firstResponseTimeoutRules: '切组规则',
    firstResponseTimeoutLimitHint: '收到有效响应后不再切组。累计最多等待 120 秒；若单组设置更长，则以最长设置为上限，最多 600 秒。',
    firstResponseTimeoutScopeHint: '仅适用于无状态推理请求。首选分组设为 0 时，本次请求不自动切组；切组开始后，后续设为 0 的分组沿用首选分组的等待时间。绑定账号、带上游会话、图片、视频及 WebSocket 请求不适用。',
    dragRoute: '拖动调整路由优先级',
    addRoute: '添加路由',
