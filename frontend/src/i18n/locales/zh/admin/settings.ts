export default {
      ...
      features: {
        ...
        welfare: {
          title: '福利系统',
          description: '控制用户端福利中心，以及每日签到、新用户首购礼包、VIP 福利三个模块。新用户首购礼包会在符合条件的首次积分购买到账后自动发放。',
          enabled: '启用福利系统',
          enabledHint: '关闭后用户端福利入口隐藏，所有福利领取接口拒绝访问。',
          dailyEnabled: '启用每日签到福利',
          dailyEnabledHint: '开启后用户每天可随机领取一次抵用券，连续签到达标后可领取大奖。',
          newUserTrialEnabled: '启用新手试用',
          newUserTrialEnabledHint: '允许符合条件的新用户在账户积分为 0 时使用一小段 API 专用试用预算。',
          newUserTrial: '新手试用',
          newUserTrialHint: '这份积分不会进入账户积分，只能由真实网关请求消费。',
          newUserTrialQuota: '试用积分',
          newUserTrialSuccessReward: '调用成功奖励积分',
          newUserTrialSiteLimit: '全站每日试用消耗上限',
          newUserTrialIpLimit: '同 IP 每日激活账号上限',
          rechargeEnabled: '启用新用户首购礼包',
          rechargeEnabledHint: '开启后，新用户符合条件的首次积分购买到账成功时可自动获得配置的礼包积分。',
          rechargeReward: '新用户首购礼包',
          rechargeRewardHint: '礼包自动到账，不需要用户手动领取；默认不与首次充值档位赠送叠加。',
          firstRechargeBonusAmount: '礼包赠送积分',
          firstRechargeBonusAmountHint: '用户首次购买任意正积分后发放；填 0 表示不发放。',
          firstRechargeBonusValidDays: '注册后有效天数',
          firstRechargeBonusValidDaysHint: '填 0 表示不限制注册时间；大于 0 时仅注册后 N 天内首次购买可领。',
          firstRechargeBonusStackMonthly: '允许叠加首次充值赠送',
          firstRechargeBonusStackMonthlyHint: '关闭时，首笔购买如果已经获得首次充值档位赠送，则不再额外发礼包。',
          ...
          rewardMin: '最小刀数',
          rewardMax: '最大刀数',
          rewardHint: '发放到用户抵用券；消费时优先扣抵用券，再扣普通积分。',
          minAccountAgeHours: '注册后多少小时可签到',
          minAccountAgeHoursHint: '填 0 表示注册后立即可领取每日签到奖励。',
          voucherValidDays: '抵用券有效天数',
          voucherValidDaysHint: '填 0 表示不过期；大于 0 时从领取时间开始计算，到期后自动不再可用。',
        ...
        leaderboardDailyReward: {
            title: '排行榜奖励玩法',
            description: '设置上周前 10 Token 消耗榜的奖励模式：关闭、红包或抽奖。',
            minAccountAgeDays: '排行榜最低注册天数',
            minAccountAgeDaysHint: '用户注册满指定天数后才可查看排行榜和领取排行榜奖励；填 0 表示注册后立即开放。',
            enabled: '启用每周奖励',
            enabledHint: '旧版兼容开关；新配置以奖励模式为准。',
            mode: '奖励模式',
            modeHint: '关闭时只展示上周前 10 消耗；红包可由前 10 各领一次；抽奖从前 10 中开出 1 人。',
            modes: {
              disabled: '关闭奖励',
              redPacket: '红包模式',
              lottery: '抽奖模式',
            ...
            minTotalActualCostHint: '低于该门槛时仍展示上周前 10，但不开放奖励领取或开奖。',
            rank1Amount: '旧版名次 1',
            rank2Amount: '旧版名次 2',
            rank3Amount: '旧版名次 3',
            redPacketPoolAmount: '红包总池',
            redPacketMinAmount: '单个最小金额',
            redPacketMaxAmount: '单个最大金额',
            lotteryAmount: '抽奖金额',
            lotteryCron: '开奖 Cron',
            lotteryCronHint: '按服务端时区解释，例如 0 12 * * 4 表示每周四 12:00。',
          },
        riskControl: {
          title: '风控中心',
        ...
        affiliate: {
          title: '邀请赚钱',
          description: '老用户邀请新用户注册，被邀请人首次充值后双方自动获得固定奖励；积分购买返利仍可按比例发放。默认关闭。',
          enabled: '启用邀请赚钱',
          enabledHint: '关闭后用户菜单中的邀请页面入口隐藏、注册时忽略邀请码、新购买不再产生返利。已有返利积分仍可转入账户积分。',
          rebateRate: '全局返利比例',
          rebateRateHint: '购买后返给邀请人的默认比例（0-100%，例如填写 10 表示返利 10%）。',
          apiCallRewardAmount: '首次充值双方奖励金额',
          apiCallRewardAmountHint: '被邀请用户首次充值后，邀请人和被邀请人各自自动获得的固定奖励金额。0 = 不启用该奖励。',
      ...
      defaults: {
        ...
        description: '新用户的默认值',
        defaultBalance: '默认积分',
        defaultBalanceHint: '新用户的初始积分',
        affiliateRebateRate: '邀请赚钱比例',
        affiliateRebateRateHint: '购买积分后返给邀请人的比例（0-100%，例如填写 10 表示返利 10%）',
      ...
      purchase: {
        title: '积分购买/订阅页面',
        description: '在侧边栏展示“积分购买/订阅”入口，并在页面内通过 iframe 打开指定链接',
        enabled: '显示积分购买/订阅入口',
      ...
      payment: {
        ...
        maxAmount: '最高金额',
        dailyLimit: '每日限额',
        balanceRechargeMultiplier: '积分到账倍率',
        balanceRechargeMultiplierHint: '用户每支付 1 CNY 可获得多少积分',
        balanceRechargePreview: '预览：1 CNY = {usd} 积分',
        rechargeFeeRate: '购买手续费率',
        rechargeFeeRateHint: '用户购买积分时额外收取的手续费百分比，0 表示不收取手续费',
        rechargeFeePreview: '预览：购买 100 元，手续费 {fee} 元',
        rechargePackages: '积分档位',
        rechargePackagesHint: '用户只能选择启用中的档位；每档支付金额最低 5 元，到账积分可高于支付金额。',
        rechargePackageAdd: '新增档位',
        rechargePackageRemove: '删除',
        rechargePackageEnabled: '启用',
        rechargePackageLabel: '名称',
        rechargePackageLabelPlaceholder: '如：首购体验包',
        rechargePackagePayAmount: '支付金额',
        rechargePackageCreditedAmount: '到账积分',
        rechargePackageBonusAmount: '赠送',
        rechargePackageSortOrder: '排序',
        faqItems: '常见问题',
        faqItemsHint: '显示在用户购买页右侧的常见问题，可自行调整标题、内容和顺序。',
        faqAdd: '添加问题',
        faqItemIndex: '问题 {index}',
        faqTitle: '标题',
        faqTitlePlaceholder: '例如 额度与计费规则',
        faqBody: '内容',
        faqBodyPlaceholder: '输入用户展开后看到的说明内容',
        faqMoveUp: '上移',
        faqMoveDown: '下移',
        faqEmpty: '暂未配置常见问题，保存时会使用默认问题。',
        orderTimeout: '订单超时时间',
      ...
      authSourceDefaults: {
        title: '认证来源默认值',
        description: '按注册来源配置新用户默认积分、并发、订阅与授权策略。',
