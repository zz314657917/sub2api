...
describe('home page visual direction', () => {
  it('uses the compact coding-solution hero copy', () => {
    expect(zhLocale).toContain("heroEyebrow: 'AI 接入中枢'")
    expect(zhLocale).toContain("heroTitleTop: 'mapleAI'")
    expect(zhLocale).toContain("heroTitleBottom: '智能解决方案'")
    expect(zhLocale).toContain("heroDescription: '统一接入模型能力，按需付费'")
    expect(zhLocale).toContain("heroProofGateway: '统一 API 网关'")
    expect(zhLocale).toContain("unifiedGateway: '统一接入'")
    expect(zhLocale).toContain("unifiedGatewayDesc: '一把密钥接入编码模型与上游能力。'")
