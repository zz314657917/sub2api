import { describe, expect, it } from 'vitest'
import { readFileSync } from 'node:fs'
import { resolve } from 'node:path'

const styleCss = readFileSync(resolve(process.cwd(), 'src/style.css'), 'utf8')
const tailwindConfig = readFileSync(resolve(process.cwd(), 'tailwind.config.js'), 'utf8')

describe('shared visual theme contract', () => {
  it('defines shared warm light theme tokens', () => {
    expect(styleCss).toContain('--mc-grass')
    expect(styleCss).toContain('--mc-stone')
    expect(styleCss).toContain('--mc-dirt')
    expect(styleCss).toContain('.mc-pixel-grid')
    expect(tailwindConfig).toContain('#cc785c')
    expect(tailwindConfig).toContain('#a9583e')
    expect(tailwindConfig).not.toContain('#3f7f3f')
    expect(tailwindConfig).not.toContain('rgba(90, 141, 61')
    expect(tailwindConfig).toContain('#8b6f47')
  })

  it('keeps shared components blocky and usable across pages', () => {
    expect(styleCss).toContain('box-shadow: var(--mc-shadow-block)')
    expect(styleCss).toContain('border-color: var(--mc-border)')
    expect(styleCss).toMatch(/\.card\s*\{[\s\S]*background-color: rgba\(250, 249, 245, 0\.92\);[\s\S]*@apply dark:bg-dark-800\/80;[\s\S]*@apply rounded-lg;/)
    expect(styleCss).toMatch(/\.btn\s*\{[\s\S]*@apply rounded-md/)
    expect(styleCss).toMatch(/\.badge-purple\s*\{[\s\S]*background-color: rgba\(204, 120, 92, 0\.12\);[\s\S]*color: #a9583e;/)
    expect(styleCss).toMatch(/\.code-block\s*\{[\s\S]*background: #f5f0e8;[\s\S]*border: 1px solid #d8cec2;[\s\S]*color: #26251e;/)
    expect(styleCss).toMatch(/\.dark \.code-block\s*\{[\s\S]*@apply bg-gray-900 text-gray-100;/)
    expect(styleCss).toMatch(/\.rounded-2xl,\s*[\s\S]*\.rounded-3xl\s*\{[\s\S]*border-radius: 0\.5rem;/)
  })
})
